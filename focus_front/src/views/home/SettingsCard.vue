...
<template>
  ...
  <button type="button"
    ...
    :class="home.settingsCardCollapsed ? 'scale-100 opacity-100' : 'pointer-events-none scale-75 opacity-0'"
    :inert="!home.settingsCardCollapsed" aria-label="Show options"
  ...
  <section
    ...
    :class="home.settingsCardCollapsed ? 'pointer-events-none scale-90 opacity-0' : 'scale-100 opacity-100'"
    :inert="home.settingsCardCollapsed" aria-label="Session settings">
    ...
    <button type="button"
      ...
      aria-label="Hide options" @click="home.setSettingsCardCollapsed(true)">
    ...
    <!-- Card - Load error -->
    <p v-if="settings.loadFailed" class="mb-3.5 flex items-start gap-1 pr-6 text-tiny text-danger" role="alert">
      <svg class="mt-px size-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
        stroke-linecap="round" stroke-linejoin="round">
    ...
    <div :class="settings.canEdit ? '' : 'pointer-events-none opacity-40'">
      ...
      <div class="mt-4.5">
        ...
        <div v-else class="flex gap-2.5">
          <button v-for="background in backgrounds.items" :key="background.id" type="button"
            ...
            :class="settings.backgroundId === background.id
              ? 'border-accent'
      ...
      <div class="mt-4.5">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <span ref="eyeBreakInfo" class="relative flex">
              <button type="button"
                ...
                :aria-expanded="showEyeBreakInfo" aria-label="What this does"
                @click="eyeBreakInfoPinned = !eyeBreakInfoPinned" @mouseenter="eyeBreakInfoHovered = true"
              ...
              <p v-if="showEyeBreakInfo"
                ...
                <b class="font-semibold text-text-primary">notification</b>, so you can look far away
                for <b class="font-semibold text-text-primary">20</b> seconds.
              </p>
            </span>
          </div>
        ...
        <!-- Eye break - Intervals -->
        <div class="flex flex-wrap gap-1 transition-opacity duration-150 motion-reduce:transition-none"
          :class="settings.eyeBreakEnabled ? 'opacity-100' : 'pointer-events-none opacity-35'">
          <span v-for="minutes in settings.eyeBreakPresets" :key="minutes"
            ...
            <button type="button"
              ...
              :disabled="!settings.eyeBreakEnabled" :aria-pressed="settings.eyeBreakMinutes === minutes"
            ...
            <button type="button"
              ...
              :aria-label="`Remove ${minutes} min`" @click="settings.removeEyeBreakPreset(minutes)">
          ...
          <span v-if="addingPreset"
            ...
            <input ref="presetInput" v-model="presetDraft" type="text" inputmode="numeric" maxlength="2"
              class="w-4 border-0 bg-transparent p-0 text-tiny leading-none text-text-primary outline-none"
              aria-label="New interval in minutes" @keydown.enter="confirmPreset" @keydown.esc="cancelPreset"
              @blur="confirmPreset" />
            min
          </span>

          <button v-else type="button" class="inline-flex items-center" :class="[CHIP_BASE, chipClass(false)]"
            :disabled="!settings.eyeBreakEnabled" aria-label="Add another interval" @click="startAddingPreset">
    ...
    <!-- Card - Save error -->
    <p v-if="settings.saveFailed" class="mt-4.5 flex items-start gap-2 text-tiny text-danger" role="alert">
      <svg class="mt-px size-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
        stroke-linecap="round" stroke-linejoin="round">
        <circle cx="12" cy="12" r="9" />
        <path d="M12 8v4M12 16h.01" />
